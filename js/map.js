...
function drawMap(locationX, locationY, tileX, tileY, zoom) {
  ...
  tileCoords.min_x =
    tileX - Math.ceil((locationX - svgMapRect.left) / TILE_SIZE) - BUFFER_TILES;
  tileCoords.min_y =
    tileY - Math.ceil((locationY - svgMapRect.top) / TILE_SIZE) - BUFFER_TILES;
  ...
  while (currentLocationY < svgMapRect.bottom + BUFFER_TILES * TILE_SIZE) {
    ...
    while (currentLocationX < svgMapRect.right + BUFFER_TILES * TILE_SIZE) {
      if (
        currentTileX >= 0 &&
        currentTileX < maxTiles &&
        currentTileY >= 0 &&
        currentTileY < maxTiles
      ) {
...
//initialize map
drawMap(
  (viewport.width >>> 1) - 100,
  (viewport.height >>> 1) - 170,
  5755,
  3654,
  currentZoom
);
...
function mousedown(event) {
  ...
  function mousemove(event) {
    ...
    if (viewBoxCoords.min_x < mapCoords.left + BUFFER_TILES * TILE_SIZE) {
      ...
      mapCoords.right - BUFFER_TILES * TILE_SIZE
    ) {
    ...
    if (viewBoxCoords.min_y < mapCoords.top + BUFFER_TILES * TILE_SIZE) {
      ...
      mapCoords.bottom - BUFFER_TILES * TILE_SIZE
    ) {
    ...
    if (
      viewBoxCoords.min_x >
      mapCoords.left + TILE_SIZE + BUFFER_TILES * TILE_SIZE
    ) {
      ...
      viewBoxCoords.min_x + viewBoxCoords.width <
      mapCoords.right - TILE_SIZE - BUFFER_TILES * TILE_SIZE
    ) {
    ...
    if (
      viewBoxCoords.min_y >
      mapCoords.top + TILE_SIZE + BUFFER_TILES * TILE_SIZE
    ) {
      ...
      viewBoxCoords.min_y + viewBoxCoords.height <
      mapCoords.bottom - TILE_SIZE - BUFFER_TILES * TILE_SIZE
    ) {
