...
function loadVerticalTiles(xLocation, xTile) {
    ...
    for(let yLocation = mapCoords.top, yTile = tileCoords.min_y; yLocation < mapCoords.bottom; yLocation += TILE_SIZE, yTile++) {
        svgMap.appendChild(getTileImage(xLocation, yLocation, xTile, yTile, currentZoom));
    }
}
...
function removeVerticalTiles(xLocation) {
    ...
    const imgList = Object.values(svgMap.getElementsByClassName('map-tile')).filter(img => {
...
function drawMap(xLocation, yLocation, xTile, yTile, zoom) {
    ...
    for(let tile of svgMap.getElementsByClassName('map-tile')) {
        svgMap.removeChild(tile);
    }
    ...
    let startLocationX = xLocation - Math.ceil((xLocation - rect.left) / TILE_SIZE) * TILE_SIZE;
    let startLocationY = yLocation - Math.ceil((yLocation - rect.top) / TILE_SIZE) * TILE_SIZE;    
    ...
    let yLoc = startLocationY, _yTile = startTileY;
    let xLoc = startLocationX, _xTile = startTileX;
...
function mousedown(event) {
    ...
    let prevX = event.clientX, prevY = event.clientY;

    function mousemove(event) {
        let newX = event.clientX, newY = event.clientY;
        let dx = newX - prevX, dy = newY - prevY;        
        ...
        if(viewBoxCoords.min_x + viewBoxCoords.width < mapCoords.right - TILE_SIZE) {
        ...
        if(viewBoxCoords.min_y + viewBoxCoords.height < mapCoords.bottom - TILE_SIZE) {
...
/*svgMap.addEventListener("click", function(event) {
    ...
    alert(tile.getAttribute("x") + " " + tile.getAttribute("y") +" " + tile.getAttribute('href'));
});*/